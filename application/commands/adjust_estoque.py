...
@dataclass
class AdjustEstoqueCommand:
    insumo_id: int
    quantidade: int


class AdjustEstoqueUseCase:
    def __init__(self, insumo_repository: InsumoRepository):
        self.insumo_repository = insumo_repository

    async def execute(self, command: AdjustEstoqueCommand) -> Insumo | None:
        insumo = await self.insumo_repository.find_by_id(command.insumo_id)

        if insumo is None:
            return None

        insumo.ajustar_estoque(command.quantidade)
